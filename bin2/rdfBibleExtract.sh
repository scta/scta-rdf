...
if [ "$scope" == 'all' ]; then
	...
	do
		...
		saxon "-s:/$base/bible_text/Biblia_Sacra_Vulgata/Biblia_Sacra_Vulgata.xml" "-xsl:$base/scta-rdf/xsl_stylesheets/rdf_bsvQuick_quotes_conversion.xsl" "-o:$base/scta-rdf/build/quotations/bsvquotations/$bookid.rdf" lbpworkname=$bookid;
	done
else
