...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template match="tei:person">
    ...
    <xsl:variable name="person-type">
      <xsl:value-of select="./parent::tei:listPerson/@type"/>
    </xsl:variable>
    <rdf:Description rdf:about="http://scta.info/resource/{$id}">
      ...
      <xsl:if test="./tei:note[@type = 'dbpedia-url']">
        <xsl:variable name="dbpedia-url">
          <xsl:value-of select="./tei:note[@type = 'dbpedia-url']"/>
        </xsl:variable>
        <owl:sameAs rdf:resource="{$dbpedia-url}"/>
      </xsl:if>
      <xsl:if test="./tei:note[@type = 'wikidata-id']">
      ...
      <xsl:if test="./tei:note[@type = 'order']">
        <xsl:variable name="order">
          <xsl:value-of select="./tei:note[@type = 'order']"/>
        </xsl:variable>
        <sctap:personType rdf:resource="http://scta.info/resource/{$order}"/>
      </xsl:if>
      ...
      <xsl:for-each
        select="document($workscitedrdf)//sctap:workAuthor[@rdf:resource = concat('http://scta.info/resources/', $id)]">
        <xsl:variable name="itemid">
          <xsl:value-of select="./parent::rdf:Description/@rdf:about"/>
        </xsl:variable>
